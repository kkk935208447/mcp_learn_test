...
"""
一个示例展示了如何从推理模型（如DeepSeekR1）生成结构化输出。思考过程不会受到用户提供的JSON架构的指导。只有最终输出会是结构化的。

要运行此示例，您需要启动带有推理解析器的 vLLM 服务器。 --reasoning-parser:
...
这个示例演示了如何使用 OpenAI Python 客户端库从推理模型生成聊天完成内容。
"""
